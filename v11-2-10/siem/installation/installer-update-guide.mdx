...
This guide explains the procedure to recover and reinstall the UTMStack Installer using the automated recovery script provided by UTMStack.

<Warning>
  **Important:** This procedure should only be used if you are experiencing issues receiving automatic updates.
</Warning>

## The script described in this guide does **not** perform a manual system upgrade. Instead, it downloads and reinstalls the latest available version of the UTMStack Installer and restores the automatic update service.

This process is intended as a recovery mechanism when the installer or automatic update service is not functioning correctly. Running the script replaces the existing installer and reinstalls the update service, allowing the system to resume receiving automatic updates normally.

If your system is already receiving automatic updates without issues, this procedure is not required.
