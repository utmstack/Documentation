...
<CardGroup cols={2}>
  <Card title="Limit Data Scope" icon="pi pi-filter" iconAlign="left" content="- Narrow `dataTypes` selection     - Use specific index patterns     - Reduce time windows when possible" />
  
  <Card title="Efficient Expressions" icon="pi pi-tachometer-alt" iconAlign="left" content="- Put cheapest checks first     - Avoid complex string operations     - Cache repeated calculations" />
  
  <Card title="Smart Correlation" icon="pi pi-link" iconAlign="left" content="- Limit `afterEvents` searches     - Use reasonable count thresholds     - Set appropriate time windows" />
  
  <Card title="Proper Deduplication" icon="pi pi-copy" iconAlign="left" content="- Include key identifying fields     - Avoid time-based fields     - Balance between noise and visibility" />
