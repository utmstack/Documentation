...
<ParamField path="param" type="string" required>
  Elasticsearch index pattern to search (typically \"v11-alert-*\")
</ParamField>
...
def get_property_values_with_count(field, filters=None, index="v11-alert-*", top=10, order_by_count=True, sort_asc=False):
...
- **200** (OK): Successfully retrieved property values and counts
- **400** (Bad Request): Invalid field name, malformed filters, or invalid parameters
